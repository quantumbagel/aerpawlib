...
class CommandHandle:
    ...
    def time_remaining(self) -> Optional[float]:
        return (
            max(0.0, self._timeout - self.elapsed_time)
            if self._timeout
            else None
        )
...
class Vehicle:
    ...
    def __init__(
        ...
        self._connection_handler = ConnectionHandler(
            self, oeo_client=oeo_client, heartbeat_timeout=_HEARTBEAT_TIMEOUT
        )
    ...
    async def _on_connection_lost(self, event):
        ...
        if self._connected:
            logger.critical(
                f"Connection lost! Reason: {event.reason.name} - {event.message}"
            )
            self._connected = False
            self._running = False
                ...
                if not self.connection_healthy:
                    ...
                    if self._oeo_platform is not None:
                        try:
                            ...
                            logger.error(
                                f"Failed to send OEO disconnect message: {e}"
                            )
...
class Drone(Vehicle):
    ...
    async def connect(
        ...
        result = await super().connect(
            timeout, auto_reconnect, retry_count, retry_delay
        )
        ...
        handle = CommandHandle(
            ...
            progress_getter=lambda: {
                ...
                "altitude_remaining": max(
                    0.0, altitude - self.state.relative_altitude
                ),
            },
        )
        handle._start()
    ...
    async def set_velocity(
        ...
        handle = CommandHandle(
            ...
            progress_getter=lambda: {
                ...
                "duration": duration,
                "time_remaining": max(0.0, duration - (time.time() - start)),
                "velocity": velocity,
            },
        )
        handle._start()
