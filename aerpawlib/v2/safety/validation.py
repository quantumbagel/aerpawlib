...
def _validate_number(
    ...
    *,
    ...
    if value is None:
        return SafetyCheckResult.ok() if allow_none else SafetyCheckResult.fail(
            violation_type, f"{name} cannot be None"
        )

    if math.isnan(value) or math.isinf(value):
        return SafetyCheckResult.fail(violation_type, f"{name} is invalid (NaN or Inf)")

    if positive and value <= 0:
        return SafetyCheckResult.fail(violation_type, f"{name} must be positive")

    if min_val is not None and value < min_val:
        return SafetyCheckResult.fail(violation_type, f"{name} {value} is below minimum {min_val}")

    if max_val is not None and value > max_val:
        return SafetyCheckResult.fail(violation_type, f"{name} {value} exceeds maximum {max_val}")
...
def validate_coordinate(coord, name: str = "coordinate") -> SafetyCheckResult:
    ...
    if coord is None:
        return SafetyCheckResult.fail(SafetyViolationType.INVALID_COORDINATE, f"{name} cannot be None")
    ...
    for val, what, min_v, max_v in [(lat, "latitude", -90, 90), (lon, "longitude", -180, 180)]:
...
def validate_altitude(altitude: float, name: str = "altitude") -> SafetyCheckResult:
    ...
    return _validate_number(altitude, name, SafetyViolationType.INVALID_ALTITUDE, min_val=-100, max_val=10000)


def validate_speed(speed: float, limits: SafetyLimits, name: str = "speed") -> SafetyCheckResult:
    """Validate a speed value against safety limits."""
    result = _validate_number(speed, name, SafetyViolationType.INVALID_SPEED, allow_none=True, min_val=0)
...
def validate_velocity(velocity, limits: SafetyLimits, name: str = "velocity") -> SafetyCheckResult:
    """Validate a velocity vector against safety limits."""
    if velocity is None:
        return SafetyCheckResult.fail(SafetyViolationType.INVALID_PARAMETER, f"{name} cannot be None")
...
def validate_timeout(timeout: float, name: str = "timeout") -> SafetyCheckResult:
    ...
    return _validate_number(timeout, name, SafetyViolationType.INVALID_PARAMETER,
                           allow_none=True, positive=True, max_val=3600)
...
def validate_tolerance(tolerance: float, name: str = "tolerance") -> SafetyCheckResult:
    ...
    result = _validate_number(tolerance, name, SafetyViolationType.INVALID_PARAMETER,
                             allow_none=True, positive=True)
...
def clamp_velocity(velocity, limits: SafetyLimits):
    ...
    if h_speed > limits.max_speed and h_speed > 0:
        ...
        logger.warning(f"Horizontal velocity scaled: {h_speed:.1f}m/s -> {limits.max_speed}m/s")
...
async def run_preflight_checks(vehicle: "Vehicle", limits: SafetyLimits) -> PreflightCheckResult:
    ...
    if config_errors:
        result.add_check("config", SafetyCheckResult.fail(
            SafetyViolationType.INVALID_PARAMETER,
    ...
    if limits.require_gps_fix:
        ...
        satellites = getattr(vehicle.gps, 'satellites', getattr(vehicle.gps, 'satellites_visible', 0))
        gps_ok = has_fix and satellites >= limits.min_satellites

        if not gps_ok:
            result.add_check("gps", SafetyCheckResult.fail(
    ...
    if battery_pct <= limits.critical_battery_percent:
        result.add_check("battery", SafetyCheckResult.fail(
    ...
    if hasattr(vehicle, '_connected'):
        if not vehicle._connected:
            result.add_check("connection", SafetyCheckResult.fail(
                SafetyViolationType.INVALID_PARAMETER,
